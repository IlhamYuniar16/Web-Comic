<template>
  ...
  <section class="bg-gray-200 mt-10 md:mt-25">
      ...
      <section class="pt-10 container mx-auto max-w-6xl px-4">
        ...
        <!-- Comic Terbaru -->
        <main class="mt-5 pb-20 bg-white shadow rounded-md p-5">
          <h1 class="font-semibold text-xl mb-5 text-gray-800">Manhwa Populer</h1>
...
async function getData() {
  try {
    const res = await axios.get("https://www.sankavollerei.com/comic/kiryuu/popular");
    latestData.value = res.data.results || [];
  } catch (err) {
...
function openPage(slug) {
  ...
  router.push(`/comic/kiryuu/manga/${cleanSlug}`);
}

const filteredData = computed(() => {
  return latestData.value.filter((item) =>
...
function toggleDarkMode() {
  ...
  document.documentElement.classList.toggle("dark", openDarkMode.value);
}
