<template>
  <header class="shadow-md bg-white fixed w-full top-0 left-0 z-50">
    ...
    >
      ...
      <a href="#rekom" class="nav-btn">Rekomendasi</a>
  ...
  <section class=" bg-gray-100 mt-10 md:mt-25">
    <!-- Section Intro -->
    <section class="pt-10 container mx-auto max-w-6xl px-4">
      <main class="bg-white shadow rounded-md overflow-hidden">
        ...
        <div class="p-5 text-gray-700 leading-relaxed">
          <p>
            LynxxComic situs baca komik, manga, dan manhwa terpopuler dalam Bahasa Indonesia.
            Tanpa iklan mengganggu, kamu cukup scroll untuk membaca chapter berikutnya — mudah, cepat, dan gratis.
          </p>
        </div>
      </main>
  
      <!-- Comic Terbaru -->
      <main class="mt-5 bg-white shadow rounded-md p-5">
        <h1 class="font-semibold text-xl mb-5 text-gray-800">Comic Terbaru</h1>
        ...
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
          ...
          >
            ...
            <span class="block text-xs text-gray-500 mt-1">{{ items.latestChapter || 'Chapter tidak tersedia' }}</span>
          </div>
        </div>
      </main>
    </section>
    ...
    <!-- Comic Rekomendasi -->
    <section id="rekom" class="pt-5 pb-25 container mx-auto max-w-6xl px-4">
      <main class="mt-10 bg-white shadow rounded-md p-5">
        <h1 class="font-semibold text-xl mb-5 text-gray-800">Comic Rekomendasi</h1>
...
async function getData() {
  try {
    const [latestRes, recomRes, weeklyRes] = await Promise.all([
      axios.get('https://www.sankavollerei.com/comic/kiryuu/home'),
      axios.get('https://www.sankavollerei.com/comic/kiryuu/recommendations'),
      axios.get('https://www.sankavollerei.com/comic/kiryuu/top-weekly'),
    ])

    latestData.value = latestRes.data.trending || []
    recomData.value = recomRes.data.results || []
    weeklyData.value = weeklyRes.data.recommendations || []
  } catch (err) {
    console.error(err)
...
const searchData = computed(() => {
  ...
  if (!search.value) return combined
